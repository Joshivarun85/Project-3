...
from sqlalchemy import event  # Correct import for event
...
@login_required
def dashboard():
    usr = User.query.all() or []
    packg = Package.query.all() or []
    hotl = Hotel.query.all() or []
    trans = Transport.query.all() or []
    cont = Contact.query.all() or []
    feedb = Feedback.query.all() or []
    paym = Payment.query.all() or []

    return render_template(
        'dashboard.html',usr=usr, packg=packg, hotl=hotl, cont=cont, feedb=feedb, paym=paym, trans=trans
    )
...
def hotel():
    if request.method == 'POST':
        try:
            ...
            star_type = request.form.get('hotel_type')

            print(f"DEBUG: Check-in Date: {checkin_date}")
            print(f"DEBUG: Cost: {cost}")
            print(f"DEBUG: Star Type: {star_type}")

            # Validate inputs
            if not checkin_date:
                print("DEBUG: Check-in date is missing.")
                flash("Check-in date is required.")
                return redirect(url_for('hotel'))
            if not cost:
                print("DEBUG: Cost is missing.")
                flash("Cost is required.")
                return redirect(url_for('hotel'))
            if not star_type:
                print("DEBUG: Star type is missing.")
                flash("Star type is required.")
                return redirect(url_for('hotel'))

            # Retrieve package data
            pk_days = db.session.query(Package).filter(current_user.email == Package.email).all()
            print(f"DEBUG: Packages found: {pk_days}")

            if not pk_days:
                flash("No packages found for the user.")
                return redirect(url_for('hotel'))

            # Process package data
            last_package = pk_days[-1]
            pdays = int(last_package.numOfDays)
            place = last_package.place
            print(f"DEBUG: Last Package Days: {pdays}, Place: {place}")
            ...
            checkout_date = startdate + timedelta(days=pdays)
            print(f"DEBUG: Checkout Date: {checkout_date}")
            ...
            print("DEBUG: Accommodation successfully added.")
            flash('Travel accommodation data added successfully.')
            return redirect(url_for('transport'))
        except Exception as e:
            print(f"DEBUG: Error encountered: {e}")
            flash("Accommodation could not be added. Check details entered.")
            return redirect(url_for('hotel'))
